...
if __name__ == '__main__':
    try:
        if len(sys.argv) == 2:
            ...
            elif sys.argv[1] == 'occupied_rooms':
                for user_room in query.get_all_occupied_rooms():
                    print(f"{user_room[7]} occupied during from {user_room[1]} to {user_room[2]} by {user_room[9]}")
            ...
            if sys.argv[1] == 'occupy' and sys.argv[4].isdigit() and sys.argv[5].isdigit():
                start_time, end_time = utils.date_validator(sys.argv[2], sys.argv[3])
                user_id = sys.argv[4]
                room_id = sys.argv[5]
                if start_time is not False and end_time is not False:
                    query.occupied_by_user(start_time, end_time, user_id, room_id)
                    print(f"Occupied Success")
                else:
